...
float calcularReembolsoIndividual(float preco, int distancia)
{
    return (preco * distancia) / 3;
}
...
void calcularReembolso()
{
    ...
    for(i=0; i < totalViagens; i++)
    {
        viagens[i].total = calcularReembolsoIndividual(viagens[i].preco, viagens[i].distancia);
